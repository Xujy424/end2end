...
from training_v2.trainer.supervised import SupervisedTrainerV2
...
def run_rolling_supervise(args, model_class, *, rolling_windows=None, window_params=None, loss_config=None, run_name=None):
    ...
    for idx, (train_win, valid_win, test_win) in enumerate(rolling_windows, start=1):
        ...
        trainer = SupervisedTrainerV2(fold_args, model_class, run_name=f"{base_name}/rolling/window_{idx:02d}")
    ...
    out_dir = Path(args.training.perf_path).expanduser() / args.model.name / "v2" / base_name / "rolling"
