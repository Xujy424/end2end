...
from training_v2.losses import build_loss
from training_v2.optimizers import EarlyStopping, build_optimizer, build_scheduler
...
class SupervisedTrainerV2:
    ...
    def __init__(self, args, model_class, loss=None, run_name=None):
        ...
        self.perf_dir = Path(self.args.training.perf_path).expanduser() / self.args.model.name / "v2" / suffix
    ...
    def make_loader(self, start_date, end_date, *, shuffle=False, indices=None):
        ...
        if int(self.args.training.batch_size) != 1:
            raise ValueError("V2 requires batch_size=1: one daily N×T×F cross-section")
